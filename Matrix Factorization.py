...
user_feature_matrix = tf.Variable(initial_value=tf.random.truncated_normal(shape=[n_user, dim]),
                                  name='user_feature_matrix')
item_feature_matrix = tf.Variable(initial_value=tf.truncated_normal(shape=[n_item, dim]),
...
# predicted ratings
predicted_ratings = tf.reduce_sum(user_features * item_features, axis=-1)
